...
namespace ControlLocalizer
{
    ...
    public static class LanguageHelper
    {
        ...
        {
            var f = new FrmMain(); f.ShowDialog();
        }
        ...
        public static void Translate(object control)
        {
            ...
            if (control is Form)
                ...
                TranslateSearchLookUpEdit(control as SearchLookUpEdit);
        ...
        public static void Translate_frm(object control, string frm)
        {

            if (control is BarManager)
                TranslateBarManagerControl_frm(control as BarManager, frm);
            
        }

        private static void TranslateBarManagerControl_frm(BarManager barManager, string frm)
        {
            foreach (BarItem i in barManager.Items)
            {
                if (i.Caption != string.Empty)
                {
                    string name = frm + "." + i.Name;
        ...
        private static void TranslateSearchLookUpEdit(SearchLookUpEdit gridColum)
        {

            foreach (GridColumn c in gridColum.Properties.PopupView.Columns)
            {
                if (c.Caption == string.Empty) continue;

                string name = gridColum.FindForm().Name + "." + gridColum.Name + "." + c.Name;
                c.Caption = TranslateString(name, c.Caption);
            }
        ...
        private static void TranslateForm(Form form)
        {
            ...
            var list = EnumerateComponents(form).ToList();
